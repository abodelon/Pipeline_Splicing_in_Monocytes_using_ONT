---
title: "Fig3 and 4: Analysis of Isoform Switches in monocytes LPS vs 0h"
...
myDesign <- fread("ReferenceGenome/metadata/reads_manifestFile.txt",header = FALSE) %>%
  ...
  mutate(sampleID = paste(sampleID, "_salmon_quant", sep = "")) %>% 
  mutate(condition=sub("3h","", condition)) %>%
  mutate(donnor=sub("batch","HC",batch))

myDesign$condition=sub("CD14sexVivo","0h", myDesign$condition)
myDesign$condition=sub("CD14sLPS","3hLPS", myDesign$condition)
...
GeneExpression=data.table(Norm[["ONT"]][["Gene"]][Norm[["ONT"]][["Gene"]]$ID == "ENSG00000162711", ]) %>% 
  pivot_longer(cols = -ID, names_to = "sampleID", values_to = "Values") %>%
  mutate(sampleID=sub("aSteady.state_","",sub("LPS_","",sampleID))) %>%
  left_join(myDesign[,c("sampleID","condition","donnor")], by = "sampleID")

Gene=ggplot(GeneExpression, aes(x = condition, y = Values, color = condition, fill = condition)) +
    ...
    scale_fill_manual(values = c("3hLPS"="#BC3C29FF","0h"="#6F99ADFF")) +
    theme_bw() +
    theme(panel.border = element_blank(), panel.grid.major = element_blank(),
          panel.grid.minor = element_blank(), 
          axis.line = element_line(colour = "black"), 
          legend.position = "none") + ylim(c(0,1000)) + scale_y_continuous(breaks = c(0,200,400,600,800,1000))

DGE[["ONT"]][DGE[["ONT"]]$Name == "ENSG00000162711",]$padj

SavePPT(PPT, Gene, "NLRP3_Gene.pdf", 3, 4)

# DUI
iso_gene <- IsoformSwitching[["ONT"]]$isoformFeatures[, c("isoform_id", "gene_id")] %>% filter(gene_id == "NLRP3")
isoformRepIF=IsoformSwitching[["ONT"]][["isoformRepIF"]] %>%
  filter(isoform_id %in% iso_gene$isoform_id)

DIU=Expression=isoformRepIF %>% pivot_longer(cols = -isoform_id, names_to = "sampleID", values_to = "IF") %>%
  left_join(myDesign[,c("sampleID","condition","donnor")], by = "sampleID")
  
DIU$isoform_id=factor(DIU$isoform_id,
                 levels = c("ENST00000697350.1", "b8276f1b-9f90-4171-8d54-8e106acd1d37",
...
Plot=ggplot(DIU, aes(x = isoform_id, y = IF, color = condition, fill = condition)) +
  geom_bar(stat = "summary", fun = "mean",position = pd, color = "black") + 
  geom_errorbar(stat = "summary", fun.data = "mean_se",position = pd, color = "black", width = 0.2) +
  geom_point(aes(group = condition),position = pd,size = 1,color = "black") +
  scale_fill_manual(values = c("3hLPS" = "#BC3C29FF", "0h" = "#6F99ADFF")) +
  theme_bw() +
  theme(panel.border = element_blank(),panel.grid.major = element_blank(),
    panel.grid.minor = element_blank(),axis.line = element_line(colour = "black"),
...
SavePPT(PPT, Plot, "NLRP3_DIU_2.pdf", 3.75, 4)
