...
NEW_PATH="Alignments_ShortReads"
REPORT_PATH="Reports"
TRIM_PATH="Trimmed_BulkRNASeq"

mkdir "Trimmed_BulkRNASeq"
mkdir "Alignments_ShortReads"
...
do 
	...
	echo "Trimming ${file%.*} with cutadapt and aligning to the reference genome with STAR." # print an OK message
	...
	trim_galore --cores 4 -q 20 --phred33 --output_dir $TRIM_PATH --fastqc --stringency 1 --gzip --length 20 ${files[$i]}
...
echo "Trimming and alignment of short reads finished the $(date)." # print an OK message
echo "Trimming and alignment of short reads finished the $(date)." >> $REPORT_PATH/progress.txt # print the end message in the progress .txt
